...
exports.getAllPosts = (req, res, next) => {
    ...
    sql.query('SELECT topics.id,topics.title,topics.content,topics.media_url, DATE_FORMAT(topics.creationdate, "%d/%m/%Y") AS post_date_fr, topics.tag, user.pseudo, user.avatar FROM topics INNER JOIN user ON topics.author = user.id ORDER BY topics.id DESC', (error, results, fields) => {
        if (error) {
            return res.status(500).json({ error });
        }
...
//like ou dislike d'un post A FAIRE
