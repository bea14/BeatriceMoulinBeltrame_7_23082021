  ...
  <div class="maincontainer">
    ...
    <div class="userboard">
      ...
      <p class="alert-reporting">
        Vous avez écrit :
        <span class="weightbold">{{ userPostsCount().length }}</span>
        article(s)<br />
        Vous avez
        <span class="weightbold">{{ userReportedPostsCount().length }}</span>
        article(s) signalé(s)
      </p>
      <div class="table-body" tabindex="0">
        <table class="table table-striped">
          ...
          <tbody class="table-posts">
            <tr
              v-for="post in userPostsCount()"
              :key="post.id"
              :class="`${postIsReported(post.id) ? 'red' : ''}`"
            >
              ...
              <td>
                <div
                  @click="
                    $refs.modaleSuppressionPostUser.openModal();
                    postId = `${post.id}`;
                  "
                >
      ...
      <Modal ref="modaleSuppressionPostUser">
        ...
        <template v-slot:body>
          ...
          <p>
            Cliquez sur Confirmer pour confirmer la suppression et sur Annuler
        ...
        <template v-slot:footer>
          <div>
            ...
            <Bouton
              @click="$refs.modaleSuppressionPostUser.closeModal()"
              text="Annuler"
            />
          </div>
        </template>
...
export default {
  ...
  methods: {
    ...
    userReportedPostsCount() {
      const pseudo = sessionStorage.getItem("pseudo");
      const countReported = this.Posts.slice(0).filter(
        (post) => post.isreported == "O" && post.pseudo == pseudo
      );
    ...
    postIsReported(id) {
      ...
      const postReported = this.Posts.filter(
        (post) => post.isreported == "O" && post.id == id
      );
      if (postReported.length != 0) {
        postIsReported = true;
      }
