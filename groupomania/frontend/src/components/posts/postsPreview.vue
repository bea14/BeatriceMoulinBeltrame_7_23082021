      ...
      <div class="card">
        <div class="card-header">
          <div class="card-header-top">
            Proposé par
            <span
              ><strong> {{ postData.pseudo }}</strong></span
            >
            <img
          ...
          <div class="card-header-bottom">
            ...
            <div class="card-header-buttons">
              ...
              <button
                ...
                v-if="postData.pseudo == this.pseudo || userIsAdmin"
                class="card-button"
              ...
              <router-link
                :to="{ name: 'UpdatePost', params: { id: postData.id } }"
              >
                <button
                  v-if="postData.pseudo == this.pseudo"
                  class="card-button"
                  aria-label="Modifier l'article"
                >
                  <i class="fas fa-edit" aria-hidden="true"> &nbsp;&nbsp; </i>
                  <span class="sr-only"> Modifier l'article </span>
                </button>
              </router-link>
              <!--Bouton de signalement -->
              <button
                class="card-button"
                @click.prevent="reportPost(postData.id)"
                aria-label="Signaler l'article"
              >
                <i class="fa fa-exclamation-triangle" aria-hidden="true">
                  &nbsp;&nbsp;
                </i>
                <span class="sr-only"> Signaler l'article </span>
              </button>
            </div>
        ...
        <div class="card-body">
          ...
          <div class="card-body-video" v-if="thereIsVideo">
            <video
              aria-label="vidéo partagée par l'utilisateur, cliquer ici pour la lire"
              controls
            >
              <source type="video/mp4" :src="`${postData.media_url}`" />
          ...
          <div class="card-body-title">
            ...
            <span
              v-if="postData.tag != null || postData.tag != ''"
              :class="{ badge: postData.tag }"
            >
              {{ postData.tag }}
            </span>
          </div>
          ...
          <div class="card-body-text">
            <p
              v-if="checkLink(postData.content)"
              class="card-body-text-content"
            >
              <a :href="`${postData.content}`" target="_blank">{{
                postData.content
              }}</a>
            </p>
...
export default {
  ...
  methods: {
    ...
    likedislikealreadyvoted() {
      ...
      const userLike = this.allLikes.filter(
        (item) =>
          item.user == userId &&
          item.topic == this.postData.id &&
          item.likedislike == 1
      );
      //Regarde si  un user a un dislike sur ce post
      const userDislike = this.allLikes.filter(
        (item) =>
          item.user == userId &&
          item.topic == this.postData.id &&
          item.likedislike == -1
      ...
      const userNeutral = this.allLikes.filter(
        (item) =>
          item.user == userId &&
          item.topic == this.postData.id &&
          item.likedislike == 0
    ...
    updateLikeDislikePost(vote) {
      ...
      axios
        .post(
          `http://localhost:3000/api/topics/` + postId + `/like`,
          likedislikevote,
          {
            headers: {
              Authorization: `Bearer ${token}`,
    ...
    likeCount(id) {
      ...
      const likeCount = this.allLikes.filter(
        (like) => like.topic == id && like.likedislike == 1
      );
      const likeCountId = likeCount.findIndex((like) => like.pseudo === pseudo);
      likeCountId != -1
        ? (document.getElementById(selectedpost).style.color = "blue")
        : "";
      return likeCount;
    },
    ...
    dislikeCount(id) {
      ...
      selectedpost += id;
      let pseudo = sessionStorage.getItem("pseudo");
      const dislikeCount = this.allLikes.filter(
        (dislike) => dislike.topic == id && dislike.likedislike == -1
      );
      const dislikeCountId = dislikeCount.findIndex(
        (dislike) => dislike.pseudo === pseudo
      );
      dislikeCountId != -1
        ? (document.getElementById(selectedpost).style.color = "red")
        : "";
      return dislikeCount;
    },
    ...
    createComment() {
      ...
      axios
        .post(
          `http://localhost:3000/api/topics/` + postId + `/comments`,
          newComment,
          {
            headers: {
              Authorization: `Bearer ${token}`,
    ...
    reportPost(id) {
      ...
      axios
        ...
        .then((response) => {
          console.log(response);
          alert(
            "Ce message a bien été signalé, les modérateurs ont été avertis"
          );
          window.location.reload();
        })
        .catch(function (error) {
